...
const resolveDatabaseTarget = (value?: string): DatabaseTarget => {
  ...
  throw new Error(
    `Invalid database target \"${value}\". Use one of: dev, prod`,
  );
};
...
if (databaseTarget === "prod" && !hasProdConfirmation) {
  throw new Error(
    "Refusing to run against production without --confirm-prod. Example: npx tsx scripts/fetch-jobs.ts prod --confirm-prod",
  );
}
...
(async () => {
  const inserted = await fetchAndInsertJobs(databaseUrl);
  console.log(`Inserted ${inserted} jobs into the ${databaseTarget} database`);
})();
